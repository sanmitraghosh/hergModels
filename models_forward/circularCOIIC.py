...
model_file = os.path.join(root, 'beattie-2017-ikr-markov.mmt')
...
class ForwardModel(pints.ForwardModel):
    ...
    def __init__(self, protocol, temperature, sine_wave=False, logTransform=False):
        ...
        self.simulation.set_tolerance(1e-9, 1e-9)
	...
	#parameters[0],parameters[2],parameters[4],parameters[6]=np.exp([parameters[0],parameters[2],parameters[4],parameters[6]])
	if self.logParam:
...
def fetch_parameters(MCMC=None):
	...
	filename = os.path.join(root,'model-1-cell-5-cmaes.txt')
	print(filename)
