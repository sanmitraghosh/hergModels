...
model_file = os.path.join(root, 'circularCCOIICIC.mmt')
...
class ForwardModel(pints.ForwardModel):
	...
	#parameters[0],parameters[2],parameters[4],parameters[6],parameters[8],parameters[10]=np.exp([parameters[0],parameters[2],parameters[4],parameters[6],parameters[8],parameters[10]])
	if self.logParam:
...
def fetch_parameters(MCMC=None):
	...
	filename = os.path.join(root,'model-5-cell-5-cmaes.txt')
	print(filename)
